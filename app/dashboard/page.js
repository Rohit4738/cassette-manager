...
import { BookOpen, Calendar, Plus, LogOut, ChevronDown, ArrowRight, Sun, Moon } from 'lucide-react'
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    load()
    const handleClick = (e) => { if (menuRef.current && !menuRef.current.contains(e.target)) { setMenuOpen(false); setThemeOpen(false) } }
  ...
  const toggleMode = async () => {
    const currentTheme = profile?.theme || 'solar-dark'
    const base = currentTheme.replace('-dark','').replace('-light','')
    const newDark = !isDark
    applyTheme(base, newDark)
  }
    ...
    <div style={{ minHeight:'100vh', background:'var(--bg)', fontFamily:'Inter,sans-serif', color:'var(--text)' }}>
        ...
        <div style={{ maxWidth:'1100px', margin:'0 auto', padding:'0 1.25rem', display:'flex', alignItems:'center', justifyContent:'space-between', height:'56px' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'1.25rem' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'.55rem' }}>
              ...
              <span style={{ fontWeight:700, fontSize:'.95rem', letterSpacing:'-.01em' }} className="hide-mobile">Cassette Manager</span>
            </div>
            <div style={{ display:'flex', gap:'.2rem' }}>
              <Link href="/subjects" className="pill-nav"><BookOpen size={14}/> <span className="hide-mobile">Subjects</span></Link>
              <Link href="/calendar" className="pill-nav"><Calendar size={14}/> <span className="hide-mobile">Calendar</span></Link>
            </div>
          </div>

          <div ref={menuRef} style={{ position:'relative', display:'flex', alignItems:'center', gap:'.5rem' }}>
            {/* Dark/light toggle */}
            <button onClick={toggleMode} className="btn-ghost" style={{ padding:'.45rem .55rem', borderRadius:'9px' }} title="Toggle light/dark">
              {isDark ? <Sun size={16}/> : <Moon size={16}/>}
            </button>

            {/* Profile */}
            <button onClick={()=>{ setMenuOpen(!menuOpen); setThemeOpen(false) }} style={{ display:'flex', alignItems:'center', gap:'.45rem', background:'var(--surface2)', border:'1px solid var(--border2)', borderRadius:'10px', padding:'.38rem .45rem .38rem .4rem', cursor:'pointer' }}>
              ...
              <span style={{ fontWeight:600, fontSize:'.82rem', color:'var(--text2)' }} className="hide-mobile">{profile?.username}</span>
      ...
      <main style={{ maxWidth:'1100px', margin:'0 auto', padding:'2rem 1.25rem' }}>
        ...
        <div className="fade-up" style={{ marginBottom:'1.75rem' }}>
          <h1 style={{ fontSize:'2rem', fontWeight:800, letterSpacing:'-.03em', lineHeight:1.2 }} className="mobile-text">
            {greeting}, <span style={{ color:'var(--accent)' }}>{profile?.username}</span>
          </h1>
          <p style={{ color:'var(--muted)', marginTop:'.25rem', fontSize:'.85rem' }}>
        ...
        {/* Top bento row */}
        <div className="fade-up-2" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'.85rem', marginBottom:'.85rem' }} >
          <Link href="/subjects/new" className="card-accent" style={{ padding:'1.5rem', display:'flex', flexDirection:'column', justifyContent:'space-between', minHeight:'150px' }}>
            <div style={{ width:'34px', height:'34px', background:'rgba(0,0,0,.15)', borderRadius:'9px', display:'flex', alignItems:'center', justifyContent:'center' }}>
              <Plus size={17} color="var(--accent-fg)"/>
            </div>
            <div>
              <div style={{ fontWeight:700, fontSize:'.9rem', color:'var(--accent-fg)', marginBottom:'.15rem' }}>New Subject</div>
              <div style={{ fontSize:'.75rem', color:'rgba(0,0,0,.45)' }}>Add a course</div>
            </div>
          </Link>

          <Link href="/calendar" className="card" style={{ padding:'1.5rem', display:'flex', flexDirection:'column', justifyContent:'space-between', minHeight:'150px' }}>
            <div style={{ width:'34px', height:'34px', background:'var(--accent-light)', borderRadius:'9px', display:'flex', alignItems:'center', justifyContent:'center' }}>
              <Calendar size={17} color="var(--accent)"/>
            </div>
            <div>
              <div style={{ fontWeight:700, fontSize:'.9rem', marginBottom:'.15rem' }}>Add Event</div>
              <div style={{ fontSize:'.75rem', color:'var(--muted)' }}>Set a deadline</div>
            </div>
          </Link>

          <div className="card" style={{ padding:'1.5rem', minHeight:'150px', display:'flex', flexDirection:'column', justifyContent:'space-between' }}>
            <div style={{ fontWeight:600, fontSize:'.72rem', color:'var(--muted)', letterSpacing:'.06em' }}>SUBJECTS</div>
            <div>
              <div style={{ fontWeight:800, fontSize:'2.8rem', letterSpacing:'-.04em', color:'var(--text)', lineHeight:1 }}>{subjects.length}</div>
              <div style={{ fontSize:'.75rem', color:'var(--muted)', marginTop:'.25rem' }}>active courses</div>
            </div>
          </div>

          <div className="card" style={{ padding:'1.5rem', minHeight:'150px', display:'flex', flexDirection:'column', justifyContent:'space-between' }}>
            <div style={{ fontWeight:600, fontSize:'.72rem', color:'var(--muted)', letterSpacing:'.06em' }}>DUE SOON</div>
            <div>
              <div style={{ fontWeight:800, fontSize:'2.8rem', letterSpacing:'-.04em', color:deadlines.filter(d=>d.daysLeft<=7).length>0?'var(--accent)':'var(--text)', lineHeight:1 }}>
                {deadlines.filter(d=>d.daysLeft<=7).length}
              </div>
              <div style={{ fontSize:'.75rem', color:'var(--muted)', marginTop:'.25rem' }}>this week</div>
            </div>
          </div>
        </div>

        {/* Bottom row */}
        <div className="fade-up-3" style={{ display:'grid', gridTemplateColumns:'2fr 1fr', gap:'.85rem' }} >
